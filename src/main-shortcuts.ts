  ...
  document.addEventListener("keydown", (e: KeyboardEvent) => {
    ...
    if ((e.key === "ArrowDown" || e.key === "ArrowUp") && !e.ctrlKey && !e.metaKey) {
      ...
      if (active && active.classList.contains("issue-card")) {
        ...
        if (!column) return;
        const cards = Array.from(column.querySelectorAll<HTMLElement>(".issue-card:not(.dragging)"));
...
// Module-scope `let` in `events.ts`; read here as a global alias to
// preserve the classic-script behaviour.
declare function getCurrentUndoCallback(): (() => void) | null;
