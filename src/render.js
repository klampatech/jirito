  ...
  `;
    ...
    if (checkbox) {
        checkbox.addEventListener("change", () => {
            if (checkbox.checked)
                getSelectedIds().add(issue.id);
            else
                getSelectedIds().delete(issue.id);
            updateBulkBar();
        });
    }
    return card;
...
export function startInlineRename(key, itemEl) {
    ...
    const finishRename = () => {
        ...
        if (getCurrentProject() === key) {
            ...
            if (navName)
                navName.textContent = getProjects()[key].name;
        }
    };
...
export function renderCalendarView() {
    ...
    if (!container)
        return;
    // Navigation HTML
    const navHtml = '<div class="calendar-nav"><button class="btn btn-sm btn-calendar-prev" id="calendar-prev">◀</button><span class="calendar-month-label">' +
        ...
        day.addEventListener("click", () => {
            ...
            if (filtered.length > 0) {
                const lines = filtered
                    ...
                    return ('<div style="display:flex;align-items:center;gap:6px;padding:4px 0;border-bottom:1px solid var(--border-light);"><span style="width:8px;height:8px;border-radius:50%;background:' +
                        statusColor +
...
export function renderDashboardView() {
    ...
    filtered.forEach((i) => {
        ...
        if (i.status === "done")
            byAssignee[a].done++;
        if (isOverdue(i.dueDate, i.status))
    ...
    container.innerHTML =
        '<div class="dashboard-stats"><div class="dashboard-stat-card"><div class="dashboard-stat-value">' +
            ...
            Object.entries(byStatus)
                .filter(([, v]) => v > 0)
                .map(([status, count]) => '<div class="dashboard-legend-item"><span class="dashboard-legend-dot" style="background:' +
                statusColors[status] +
                '"></span><span>' +
                statusLabels[status] +
            ...
            assignees
                .map(([name, data], idx) => '<div class="dashboard-bar-row"><span class="dashboard-bar-label">' +
                escapeHtml(name) +
            ...
            Object.entries(byType)
                ...
                const color = { story: "#2BB5A8", bug: "#E53935", task: "#34A853", epic: "#F5C842" }[type];
                return ('<div class="dashboard-bar-row"><span class="dashboard-bar-label">' +
...
export function populateAssigneeFilter() {
    const assignees = [
        ...new Set(getIssues().map((i) => i.assignee).filter(Boolean)),
    ];
...
let _iconPickerKey = "";
export function openIconPicker(projectKey, iconEl) {
...
function matchesEmojiKeyword(emoji, filter) {
...
function selectIcon(emoji) {
    ...
    const newIconEl = newItem?.querySelector(".project-icon");
    if (newIconEl) {
