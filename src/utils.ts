...
export function isSafeUrl(url: string): boolean {
  ...
  if (schemeMatch) {
    ...
    return ALLOWED_URL_SCHEMES.includes(scheme as (typeof ALLOWED_URL_SCHEMES)[number]);
  }
 ...
 */
export function lucideIcon(name: string, attrs: Record<string, string> = {}): string {
...
export function truncateDesc(str: unknown, maxChars: number): string {
  if (!str && str !== 0) return "";
...
export function generateIssueKey(projectKey: string, id: number | string): string {
...
export function getFilteredIssues(): Issue[] {
  ...
  const priorityEl = document.getElementById("filter-priority") as HTMLSelectElement | null;
...
export function populateSprintFilter(): void {
  const select = document.getElementById("sprint-filter") as HTMLSelectElement | null;
  if (!select) return;
  const currentVal = select.value;
  select.innerHTML = '<option value="all">All Sprints</option>';
  Object.values(getSprints()).forEach((s: Sprint) => {
...
export function populateSprintSelect(): void {
  const select = document.getElementById("issue-sprint") as HTMLSelectElement | null;
  if (!select) return;
  const currentVal = select.value;
...
export function updateSprintBar(): void {
  const sprintFilter = document.getElementById("sprint-filter") as HTMLElement | null;
  const manageBtn = document.getElementById("manage-sprints-btn") as HTMLElement | null;
  const sprintBar = document.getElementById("sprint-bar") as HTMLElement | null;
...
export function updateSprintProgressBar(activeSprint: Sprint): void {
  const fill = document.getElementById("sprint-progress-fill") as HTMLElement | null;
  const text = document.getElementById("sprint-progress-text") as HTMLElement | null;
  if (!fill || !text) return;
...
export function updateSprintProgress(): void {
  const activeSprint = getActiveSprint();
  if (activeSprint) {
    updateSprintProgressBar(activeSprint);
  }
