...
function getCardPosition(cardEl) {
    ...
    const cards = Array.from(col.querySelectorAll(".issue-card:not(.dragging)"));
    return {
...
export function openDetailPanel(issueId) {
    ...
    <div class="detail-field">
      ...
      <select id="detail-sprint">
        ...
        .map((s) => `<option value="${s.id}" ${issue.sprint === s.id ? "selected" : ""}>${escapeHtml(s.name)} (${formatDate(s.startDate)} - ${formatDate(s.endDate)})</option>`)
        .join("")}
      </select>
    </div>
    <div class="detail-field">
  ...
  `;
    ...
    _detailChangeHandler = (e) => {
        ...
        if (target.id === "detail-summary") {
            ...
            issue.storyPoints = target.value
            ...
            searchTimeout = setTimeout(() => {
                ...
                depResults.querySelectorAll(".dep-result-item").forEach((item) => {
                    item.addEventListener("click", () => {
                        ...
                        const typeSelect = document.getElementById("dep-type");
...
export function trackHistory(issue, field, from, to) {
    if (!issue.history)
        issue.history = [];
    issue.history.push({
        field,
...
export function deleteIssue(issueId) {
    if (!confirm("Delete this issue? You can restore it from Trash."))
        return;
...
export function addComment() {
    ...
    addActivity("Chat", "Comment added");
...
export function cloneIssue(issueId) {
    const issue = getIssues().find((i) => _matchesId(i, issueId));
    if (!issue)
        return;
...
export function initDragDrop() {
    ...
    document.querySelectorAll(".column-body").forEach((col) => {
        ...
        col.addEventListener("change", (e) => {
            ...
            if (id) {
                ...
                if (checkbox.checked)
                    getSelectedIds().add(canonId);
                else
                    getSelectedIds().delete(canonId);
                updateBulkBar();
            }
        });
    });
    ...
    document.querySelectorAll(".column-body").forEach((col) => {
        ...
        col.addEventListener("drop", (e) => {
            ...
            const targetCards = Array.from(col.querySelectorAll(".issue-card:not(.dragging)"));
            let finalIndex;
            ...
            if (sameColumn) {
                ...
                const beforeCards = Array.from(col.querySelectorAll(".issue-card:not(.dragging)")).slice(0, finalIndex);
                const afterCards = Array.from(col.querySelectorAll(".issue-card:not(.dragging)")).slice(finalIndex);
...
export function handleBulkStatusChange(e) {
...
export function handleBulkDelete() {
    if (!confirm(`Delete ${getSelectedIds().size} issues?`))
        return;
    const titles = [];
    const deletedIssues = [];
...
export function handleBulkClear() {
    getSelectedIds().clear();
 ...
 */
export function filterIssuesForCurrentProject(issues) {
    const currentProject = getCurrentProject();
    return filterIssues(issues).filter((i) => (i.projectId || currentProject) === currentProject);
}
    ...
    columns.forEach((colDef) => {
        ...
        const filtered = getIssues().filter((i) => {
            ...
            if (search &&
                ...
                !(i.desc || "").toLowerCase().includes(search))
                return false;
            if (colDef.status && i.status !== colDef.status)
                return false;
            return true;
        });
        ...
        if (!colDef.status) {
            const customFiltered = getIssues().filter((i) => i.customColumnId === colDef.id);
            ...
            customFiltered.forEach((issue) => colBody.appendChild(createCard(issue)));
            return;
        }
 ...
 */
export function getCurrentUndoCallback() {
...
export function renderSprintList() {
    const container = document.getElementById("sprint-list");
    if (!container)
        return;
    const sprints = getSprints();
    const entries = Object.values(sprints).sort((a, b) => new Date(a.startDate ?? "").getTime() - new Date(b.startDate ?? "").getTime());
    if (entries.length === 0) {
        container.innerHTML =
        ...
        btn.addEventListener("click", () => {
            ...
            if (newActive) {
                ...
                if (sprintBar) {
                    ...
                    if (nameEl)
                        nameEl.textContent = newActive.name;
    ...
    container.querySelectorAll(".sprint-delete-btn").forEach((btn) => {
        btn.addEventListener("click", () => {
            ...
            if (!id)
                return;
            if (!confirm("Delete this sprint? Issues will be unassigned from it."))
                return;
            deleteSprint(id);
            renderSprintList();
            populateSprintFilter();
